...
async fn main() -> Result<()> {
    println!("🔗 Mini API Server + Modular API Client Integration Example");
...
async fn start_mock_server() -> Result<Server> {
    let mut config = mini_api_server::Config::default();
                        ...
                        body: Some(r#"
{
  ...
  "timestamp": "{{timestamp}}"
    ...
    let mut mock_plugin = mini_api_server::plugins::mock::MockPlugin::new();
...
pub fn create_shared_config() -> (modular_api_client::Config, mini_api_server::Config) {
    ...
    let server_config = mini_api_server::Config {
        server: mini_api_server::config::ServerConfig {
