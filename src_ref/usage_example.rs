...
use mini_api_server::session::{SessionManager, MemorySessionStore};
use mini_api_server::auth::{AuthManager, BasicAuthProvider};
use mini_api_server::handlers::{json_handler, text_handler, RouterBuilder};
use mini_api_server::server::{Route, Server};
use mini_api_server::config::*;
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    println!("🚀 Starting Mini API Server Complete Example");
                        ...
                        body: Some(r#"
{
  "error": {
    ...
    config.auth.realm = Some("Mini API Server Example".to_string());
...
fn setup_example_auth() -> AuthManager {
    ...
    AuthManager::basic(users, Some("Mini API Server Example".to_string()))
}
